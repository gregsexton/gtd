...
;TODO: clojure idiom for Integer/valueOf?
(defn get-groups-date [date-str]
  (re-find #"(\d{4})-(\d{2})-(\d{2})" date-str))
(defn get-abs-year [date]
  (Integer/valueOf (nth (get-groups-date date) 1 (get-year))))
(defn get-abs-month [date]
  (Integer/valueOf (nth (get-groups-date date) 2 (get-month))))
(defn get-abs-date [date]
  (Integer/valueOf (nth (get-groups-date date) 3 (get-day))))

(defn get-groups-time [time-str]
  (re-find #"(\d{1,2}):(\d{2})(:(\d{2}))?\s*(am|pm)?" time-str))
(defn get-abs-hour [date]
  (Integer/valueOf (nth (get-groups-time date) 1)))
(defn get-abs-min [date]
  (Integer/valueOf (nth (get-groups-time date) 2)))
(defn get-abs-sec [date]
  (Integer/valueOf (nth (get-groups-time date) 4)))
